...
module holder_switch(){
    ...
    difference(){
        ...
        difference(){
            ...
            //retainer bump
            translate([-thick-wall/4,0,height*3/4+wire_height]) scale([1.1,1,2.5]) sphere(r=wall/2);
        }
        ...
        //flatten bottom
        for(i=[-1]) translate([0,0,25*i+height/2+height/2*i]) cube([50,50,50], center=true);
    }
}
...
module round_clip(rad=8, height=10, wall=3, angle=45, support=0, clamp=0, solid=0){
	...
	if(solid == 1){
		...
		*translate([-1,0,0]) difference(){
		...
		if(clamp == 1){
			rotate([0,0,1*angle/2]) translate([-rad-wall/2,0,0]) rotate([0,0,-1*angle/2]) rotate([0,0,0]) rotate([90,0,0]) translate([-m8_rad, 0, -m8_rad*4]) cylinder(r=m3_rad, h=m8_rad*6);
			rotate([0,0,1*angle/2]) translate([-rad-wall/2,0,0]) rotate([0,0,-1*angle/2]) rotate([0,0,0]) rotate([90,0,0]) translate([-m8_rad, 0, 0]) cylinder(r=m3_cap_rad, h=wall);
			rotate([0,0,-1*angle/2]) translate([-rad-wall/2,0,0]) rotate([0,0,1*angle/2]) rotate([0,0,0]) rotate([90,0,0]) translate([-m8_rad, 0, -wall]) cylinder(r=m3_nut_rad, h=wall, $fn=6);
		}
	}
}
