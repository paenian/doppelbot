...
mount_sep = 20;  //needs to match ext_offset from x-carriage.scad
...
module clamp(solid = 1, mount_screw_rad = m5_rad, m5=1){
	len = wall*5;
	facets = 6;
	if(solid==1){
		...
		if(m5 == 1){
			mirror([0,0,1]) translate([0,0,wall*1.5]) rotate([0,0,-90]) cylinder(r1=m5_sq_nut_rad, r2=m5_sq_nut_rad+1, h=len, $fn=4);
		}else{
